...
Stats=function(pool,tissue=NULL,days=NULL,animal=NULL){
	# variable declarations
	poolDay<-c();dayPool<-c();d<-c();check<-c();dayColumn<-c();tissColumn<-c();checkTiss<-c();tissuePool_pattern<-list();tissueDayPool<-list();TissueDayPool<-list()
	statsT<-c();Tissue<-c();percentageT<-c();resultsT<-c();StatsT<-list();percentageT<-list();resultsA<-c();PercentageT<-list();patA<-c();patT<-c()
	animalPool_pattern<-list();animalDayPool<-list();AnimalDayPool<-list();checkAni<-c();aniColumn<-c();percentageA<-c();statsA<-c();StatsA<-list();PercentageA<-list()
	...
	print(paste("Stats for animals:",animal[k]))
		for (j in 1:length(days)){
			...
			if (is.null(aniColumn[j])){print(paste("No",animal,"for Day",days[j]));statsA[j]=0}
	...
	return (list(tissue=TissueDayPool,results=results,statsTissue=StatsT,PercentageTissue=PercentageT,animal=AnimalDayPool,statsAnimal=StatsA,PercentageAnimal=PercentageA))
}
###############################################################################

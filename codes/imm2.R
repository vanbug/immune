...
Stats=function(pool,tissue=NULL,days=NULL,animal=NULL,dayBifur=FALSE,animalBifur=FALSE){
	...
	# brief stats calculator
		# Tissues per day segmentar
		Tissue=tissueStatsA(tissue,days,pool)
		results=Tissue$results
		if(animalBifur=="TRUE"){
...
tissueStatsA=function(tissue,days,pool){
# variable declarations
...
checkTiss<-c();tissColumn<-c();tissuePool_pattern<-list();tissueDayPool<-list();TissueDayPool<-list()
	...
	for (k in 1:length(tissue)){
			...
			for (i in 1:length(pool)){
				checkTiss[i]=length(which(pool[[i]]==tissue[k]))
				if (checkTiss[i]>=1) {tissColumn[k]=i}
			}
			# outputs empty Tissue day column, if empty
			if (is.null(tissColumn[k])) {print(paste("No",tissue,"for Day",days[k]));statsT[k]=0}
			# filters input tissue specific data for the specific pool and calculates statsT
			else {		
				tissuePool_pattern[[k]]=which(pool[[tissColumn[k]]]==tissue[k])
				tissueDayPool[[k]]=pool[tissuePool_pattern[[k]],1:length(pool)]
				statsT[k]=length(tissueDayPool[[k]][[tissColumn[k]]])
				percentageT[k]=round(((length(tissuePool_pattern[[k]])/length(pool[[tissColumn[k]]]))*100),digits=2)
				# replaces NA with 0
...
aniStatsA=function(animal,days,pool){
# variable declarations
animalPool_pattern<-list();animalPool<-list();checkAni<-c();aniColumn<-c();percentageA<-c();statsA<-c();patA<-c();resultsA<-c()
	print(paste("Stats for",length(animal),"animal"))
	for (k in 1:length(animal)){
			# tissue column detector
			for (i in 1:length(pool)){
				checkAni[i]=length(which(pool[[i]]==animal[k]))
			...
			# filters input tissue specific data for the specific pool and calculates statsT
				...
				animalPool_pattern[[k]]=which(pool[[aniColumn[k]]]==animal[k])
				animalPool[[k]]=pool[animalPool_pattern[[k]],1:length(pool)]
				statsA[k]=length(animalPool[[k]][[aniColumn[k]]])
				percentageA[k]=round(((length(animalPool_pattern[[k]])/length(pool[[aniColumn[k]]]))*100),digits=2)
				# replaces NA with 0
